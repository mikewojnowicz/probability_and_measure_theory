...
\begin{align*} 
& \forall \epsilon >0, \; \exists A \in \F : \\
& \mu(A) < \epsilon \quad \text{and} \quad f_n \to f \quad \text{uniformly on} \; A^c 	
\end{align*} 
...
\begin{question}
Why is there an asymmetry in the definitions of convergence a.e. and convergence almost uniformly?	
\end{question}


\subsection{Relations between modes of convergence}

\subsubsection{Convergence in measure vs. convergence in $L^p$}
To relate $\convergenceInMu$ and $\convergenceInLp$, we can will utilize Chebyshev's Inequality. 
...
However, if the sequence has a \textit{dominating function} (whose $p$th power is integrable), then the implication holds.
