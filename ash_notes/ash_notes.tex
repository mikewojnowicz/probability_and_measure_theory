...
\item The Reimann approach assumes that the domain is totally ordered.  In contrast, Lesbesgue integrals can be defined for functions on arbitrary spaces.
\end{enumerate}
...
%\end{document}
%
%Example \ref{ex:integrating_the_dirichlet_function} shows how Lesbesgue integration can handle functions not handled by Reimann integration.  As noted in Remark \ref{rk:{rk:dirichlet_function_has_no_reimann_integral}, a function on $[a,b]$ is Riemann integrable if and only if it is bounded and continuous almost everywhere.  Lebesgue integration can handle functions that do not meet these criteria.
...
\item If $c \geq 0$, then $\ds\int c g \wrt{\mu} = c \ds\int g \wrt{\mu}$.
\item $\ds\int (g + h) \wrt{\mu} = \ds\int g \wrt{\mu} + \ds\int h \wrt{\mu}$ .
\end{alphabate}
\label{prop:properties_of_integrals_of_simple_functions}
\end{proposition}

\begin{proof}
We prove (a) and leave the rest to the reader. By definition of simple functions, we write \[g = \ds\sum_{i=1}^r x_i I_{A_i} ,\quad \quad h = \ds\sum_{j=1}^s y_j I_{B_j}  \]	
...
So 
\begin{align*}
\ds\int h_1 \wrt{\mu}  = \ds\sum_{i=1}^r \ds\sum_{j=1}^s x_i \, \mu(A_i \cap B_j)  \leq \ds\sum_{i=1}^r \ds\sum_{j=1}^s y_i \, \mu(A_i \cap B_j) = \ds\int h_2 \wrt{\mu}.
\end{align*}
...
&= \ds\sum_{i=1}^\infty  \ds\sum_{j=1}^n y_i \, \mu(B_j \cap A_i) && \tinytext{needs justification} \\
...
\paragraphnewline{Properties of integral of non-negative Borel measurable functions}
...
\begin{itemize}
\item We first show that both quantities in \eqref{eqn:monotone_convergence_theorem_conclusion} exist.  The RHS exists because $h$ is Borel measurable (by Proposition \ref{prop:borel_measurability_closed_under_inf_sup_liminf_limsup}) and non-negative (see Remark \ref{rk:when_does_integral_of_non_negative_Borel_measurable_function_exist}).  The LHS exists since each $\int h_n$ exists (see Remark \ref{rk:when_does_integral_of_non_negative_Borel_measurable_function_exist}) and is increasing (by monotonicity; see Prop. \ref{prop:properties_of_integrals_of_non_negative_borel_measurable_functions}), and therefore has a limit (possibly $\infty$). 
...
\[ h_n \leq h \stackreltext{monotonicity}{\implies} \ds\int h_n \leq \ds\int h \stackreltext{limits preserve inequalities}{\implies} \ds\lim_{n \to \infty} \ds\int h_n \leq \ds\int h \]
...
\[ A_n := \set{x : h_n(x) \geq \alpha s(x) }\].
So
\begin{align*}
 \ds\int h_n  \stackreltext{monotonicity}{\geq} \ds\int_{A_n} h_n \stackreltext{def. $A_n$}{\geq} \ds\int_{A_n} \alpha s \stackreltext{linearity}{=} \alpha   \ds\int_{A_n} s 
 \labelit \label{eqn:intermediate_eqn_from_proof_mct} 
 \end{align*}
But note that $A_n \uparrow \Omega$, so recognizing the right hand side as a measure on $A_n$ and applying  continuity from below (see Remark \ref{rk:continuity_of_measure_applied_to_measures_given_by_integrals_of_simple_functions_over_sets}), Equation \eqref{eqn:intermediate_eqn_from_proof_mct} becomes
\[  \ds\int h_n  \geq \alpha \ds\int s \]
Now since the equality holds for all $\alpha <1$, it holds for $\alpha =1$, and so we have
\[  \ds\int h_n  \geq \ds\int s \]
Since the LHS is an upper bound , it must be greater than the least upper bound, so 
\[  \ds\int h_n  \geq \ds\int h \]
\end{itemize}
\end{proof}


The monotone convergence theorem can actually be used to make it easier to compute values for integrals of non-negative Borel measurable functions!  Let us quote \cite{folland1999real} (except with changes of notation and references)
...
\begin{align*}
\bigg\{ \omega : \max \bigg( h_1(\omega), h_2(\omega) < c \bigg) \bigg\} &=	\set{\omega :  h_1(\omega) < c } \cap \set{\omega : h_2(\omega) < c } \\
\bigg\{ \omega : \min \bigg( h_1(\omega), h_2(\omega) < c \bigg) \bigg\} &=	\set{\omega :  h_1(\omega) < c } \cup \set{\omega : h_2(\omega) < c }
\end{align*}
...
So we have expressed an arbitrary Borel measurable function as as the difference of two non-negative Borel measurable functions. Therefore, we can define its integral by 
