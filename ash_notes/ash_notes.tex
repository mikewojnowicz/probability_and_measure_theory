...
Now $B_{k+1} \subset B_k$ and $\cap_{k=1}^\infty B_k =B$, so $B_k \downarrow B$.  Since also $\mu(B_1) < \infty$ (by hypothesis and monotonicity), then we can apply continuity from above to get
\begin{align*}
...
\begin{theorem}{\textbf{Integrals are countably additive set functions.}}
...
\begin{corollary}{\textbf{Integrals as measures.}}
...
\label{cor:integrals_as_measures}
\end{corollary}
...
\begin{remark}{\textbf{Integrals as signed measures.}}
As will become clear in Section~\ref{sec:signed_measures}, another name for Thm. \ref{thm:integrals_are_countably_additive_set_functions} is \textit{integrals as signed measures}.	 Basically, if $f$ is Borel measurable but not necessarily $f \geq 0$, then $\lambda$ is a countably additive set function, but it may be negative. 
\end{remark}

\begin{remark}{\textbf{Change of measure and change of signed measure.}} \cite[pp.~24]{rudin1987real} points out an important implication of Corollary \ref{cor:integrals_as_measures}: for every measurable $g$ on $\Omega$ such that $g \geq 0$, we have 
%
\begin{align*}
\dint_\Omega g \wrt{\lambda} = \dint_\Omega g f \dmu
\labelit \label{eqn:change_of_signed_measure}
\end{align*}

This can be interpreted as a change in signed measure; specifically, as a change from measure $\mu$ to signed measure $\lambda$. In the special case where $f \geq 0$, then $\lambda$ (as defined as in the theorem statement) is a measure, and so the formula provides a mechanism for change of measure. 
...
{\tiny Let us prove \eqref{eqn:change_of_signed_measure} from Theorem \ref{thm:integrals_are_countably_additive_set_functions}.
...
\begin{notation}
Let $\lambda$ be an ``indefinite integral" \footnote{This interpretation of ``indefinite integral" is used in \cite[pp.~61]{ash2000probability}}  with respect to $\mu$, i.e. let   $\lambda$ be a countably additive set function formed from Borel measurable $f$ as in Theorem~\ref{thm:integrals_are_countably_additive_set_functions}:
%
\begin{align*}
...
The notation may make more sense if we interpret it as does \cite[pp.~24]{rudin1987real}, in light of the change of signed measure:
\begin{align*}
\dint_\Omega g \wrt{\lambda} = \dint_\Omega g f \dmu
\labelit \label{eqn:change_of_signed_measure}
\end{align*}
...
$d\lambda$ and $d\mu$; \eqref{eqn:indefinite_integral_in_differential_form} simply means that \eqref{eqn:indefinite_integral} (and therefore \eqref{eqn:change_of_signed_measure}) holds for every measurable $f$. 
...
Let $\lambda$ be an ``indefinite integral" with respect to $\mu$.\footnote{This interpretation of ``indefinite integral" is used in \cite[pp.~61]{ash2000probability}.}
\[ \text{ i.e. } \; \lambda(A) = \ds\int_A h \dmu, \quad \quad \forall A \in \F \]

Then $\lambda \ll \mu$
\[ \text{ i.e. } \; \mu(A) = 0 \implies \lambda(A) = 0  \quad \quad \forall A \in \F \]
...
The Radon-Nikodym theorem in an assertion in the opposite direction of Remark \ref{rk:indefinite_integrals_are_absolutely_continuous}:  if $\lambda \ll \mu$ (and $\mu$ is $\sigma$-finite on $\F$), then $\lambda$ is an indefinite integral with respect  to $\mu$. 
...
\begin{proof}
First recall
\[ \nu \ll \mu \stackexplain{Problem \ref{rk:signed_measure_is_absolutely_continuous_iff_its_total_variation_is_absolutely_continuous}}{\iff} |\nu| \ll \mu \]
...
\begin{proof}
By Theorem \ref{thm:integrals_are_countably_additive_set_functions}, the set function $\nu : \F \to \overline{\R}$ defined by $\nu(A) = \int_A f \wrt{\mu}$ is a signed measure.    Moreover, $\nu \ll \mu$, since indefinite integrals are absolutely continuous (see Remark \ref{rk:indefinite_integrals_are_absolutely_continuous}).  Thus, the implication follows from the name-justifying characterization of absolute continuity (see Theorem \ref{thm:justifying_the_name_absolutely_continuous}). 
\end{proof}
...
\end{array}
\]
Then\footnote{Include \cite{folland1999real} Prop. 2.23 in these notes. It is strange that \cite{ash2000probability} omits this. It can probably be obtained fairly immediately from Theorem~\ref{thm:properties_of_functions_derived_from_properties_of_integrals}.}
\[ \begin{array}{lrlr}
...
\text{So} & f&=f' \quad [\mu]-a.e. &  \tinytext{See \cite{folland1999real} Prop. 2.23}
\end{array}
\]
