...
Throughout this section, let $(X,\A)$ and $(Y,\B)$ be measurable spaces.\footnote{For this section, we follow \url{https://www.math.ucdavis.edu/~hunter/measure_theory/measure_notes_ch5.pdf}.  First, the notation for sections seems nicer - and more commonly used - than Ash's.  Second, the presentation is more modular (e.g., the proposition that sections of a measurable set are measurable in buried in Ash's general product measure proof.)}

\begin{definition}
A \textbf{measurable rectangle} is a subset $A \times B$ of $X \times Y$ where $A \in \A$ and $B \in \B$ are measurable subsets of $X$ and $Y$, respectively.\footnote{See Def.~\ref{def:cartesian_product} for the definition of the Cartesian product.}
\end{definition}
...
\begin{definition}
The \textbf{product $\sigma$-field} $\A \otimes \B$ is the $\sigma$-field on $X \times Y$ generated by the collection of all measurable rectangles.
\[ \A \otimes \B := \sigma(\{ A \times B : A \in \A, \, B \in \B \})\]
\end{definition}

\begin{definition}{\textbf{Sections.}}
Suppose that $E \subset X \times Y$.  For any $x \in X$ and $y \in Y$, we define the \textit{x-section} $E_x \subset Y$ and \textit{y-section} $E_y \subset X$ by:
\[ E_x := \set{y \in Y : (x,y) \in E}, \quad E_y := \set{x \in X : (x,y) \in E} \]
\end{definition}
...
\begin{remark}{\remarktitle{Sections of rectangles.}}
...
\label{rk:sections_of_rectangles}
\end{remark}


\begin{proposition}{\remarktitle{All sections of a measurable set are measurable.}} If $(X,\A)$ and $(Y,\B)$ are measurable spaces, and $E \in \A \otimes \B$, then $E_x \in \A$ for every $x \in X$ and $E_y \in \B$ for every $y \in Y$. 
\label{prop:sections_of_a_measurable_set_are_measurable}
\end{proposition}

\begin{proof}
We apply the ``Good sets strategy" (Sec.~\ref{sec:good_sets_strategy}).   Let
\[  \G := \{ E \subset X \times Y : E_x \in \B \text{ for all} \; x \in X \quad \text{and} \quad E_y \in \A \text{ for all} \; y \in Y \}. \]
We have
\begin{itemize}
...
\red{Warning: Here I'm switching notation for an arbitrary set in the product sigma-field, from $E$ to $F$.  Fix up!}

\begin{theorem}\textbf{(Product measure theorem.)} \cite[Thm.~2.6.2]{ash2000probability}
Consider the measure spaces $(X, \A, \mu), (Y, \B, \nu(x, \cdot))$, where 
\begin{itemize}
...
\end{itemize}
Then $\exists!$ a measure $\lambda$ on $\F := \A \otimes \B$ such that 
\[\lambda(A \times B) = \int_A \nu(x,B) \mu(dx) \quad \quad \forall A \in \A, B \in \B \]
Namely
\begin{align}
\lambda(F) = \int_X \nu(x,F_x) \mu(dx), \quad \quad \forall F \in F
\label{eqn:measure_of_set_in_product_space}
\end{align}
\label{thm:product_measure_theorem}
\end{theorem}

\begin{remark}
Theorem~\ref{thm:product_measure_theorem} says that the measure of a set $F \in \F$ is obtained as follows: for each $x \in X$, compute the measure of $F_x$, the section of $F$ at $x$, by $\nu(\explaintermbrace{parameter}{x}, \explaintermbrace{set to be measured}{F_x})$.  Then integrate this over all $x$, weighted by the measure on $X$. 
\begin{figure}[H]
\centering
...
\begin{enumerate}
\item[(1.)] If $F \in \F$, then $\nu(x, F_x)$ is Borel measurable in $x \in X$. (For a proof, see item (2) of the proof in \cite{ash2000probability}).\redfootnote{TODO: Add my proof.}
\item[(2.)] Define 
\[ \lambda(F) = \int_A \nu(x,F_x) \mu(dx), \quad \quad \forall F \in F\] 
The integral exists by (1.) {\tiny(since the integral of a non-negative Borel measurable function always exists)}.   Then 
	...
	\begin{align*}
	\lambda (\cupdot_{n=1}^\infty F_n) &\stackexplain{def}{=} \int_X \nu\bigg(x, (\cupdot_{n=1}^\infty F_n)_x \bigg) \mu(dx) \\
	& \stackexplain{(+)}{=} \int_X \sum_{n=1}^\infty \nu\bigg(x, (F_n)_x \bigg) \mu(dx) \\
	&\stackexplain{Ash Cor 1.6.4}{=} \sum_{n=1}^\infty \int_X  \nu\bigg(x, (F_n)_x \bigg) \mu(dx) \\
	&\stackexplain{def}{=} \sum_{n=1}^\infty \lambda(F_n) 
	\end{align*}
	To justify (+), we have 
	\begin{align*}
	(\cupdot_{n=1}^\infty F_n)_x &= \cupdot_{n=1}^\infty (F_n)_x && \tinytext{sections commute with unions, \eqref{eqn:sections_commute_with_complements_and_unions}} \\
	\implies \nu\bigg(x,(\cupdot_{n=1}^\infty F_n)_x\bigg) &= \nu\bigg(x,\cupdot_{n=1}^\infty (F_n)_x\bigg)  && \tinytext{substitution} \\
	&= \sum_{n=1}^\infty \nu \bigg(x, (F_n)_x\bigg) && \tinytext{countable additivity. each $(F_n)_x$ is measurable by   Prop.~\ref{prop:sections_of_a_measurable_set_are_measurable}, hence so is the union}
	\end{align*}
	\item Moreoever,
	\[\lambda(A \times B) = \int_A \nu(x,B) \mu(dx) \quad \quad \forall A \in \A, B \in \B \]
	...
	& = \int_A \nu (x, B_x) \,  \mu(dx)
	\end{align*}
	where (*) holds since by sections of rectangles (Remark~\ref{rk:sections_of_rectangles}), we have $\nu \big(x, (A \times B)_x \big) = \nu (x, B_x) \indicate{x \in A}$.
	\end{alphabate}
\end{enumerate}
