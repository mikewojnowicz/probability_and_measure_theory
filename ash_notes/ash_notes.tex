...
This definition is possible because whenever $h$ is a simple function, then so is $h \indicate{A}$ for any measurable set $A$.   Indeed, if we express $h = \sum_{i=1}^r y_i \indicate{{B_i}}$, then $h \indicate{A} = \sum_{i=1}^r y_i \indicate{A} \indicate{B_i} = \sum_{i=1}^r y_i \indicate{A \cap B_i}$, and each $A \cap B_i \in \F$.  
