...
\begin{theorem}{\remarktitle{Fatou's Lemma}}
Let $\set{f_n}$ be non-negative measurable for each positive integer $n$.  Then 
\begin{align} 
\ds\int_\Omega \bigg( \liminf_{n \to \infty} f_n \bigg) \dmu \leq \liminf_{n \to \infty} \dint_\Omega f_n \dmu
\label{eqn:fatous_lemma}
\end{align}

\end{theorem}

\begin{proof}
By definition of the limit inferior,
\[ \explaintermbrace{$:=g$}{\liminf_{n \to \infty} f_n} = \lim_{n \to \infty}  \explaintermbrace{$:=g_n$}{\inf_{m \geq n} f_n} \]	
And $g_n \uparrow g$ {\tiny (due to taking the infimum over successively smaller sets; see Prop. \ref{prop:sup_and_inf_for_subsets_are_tighter})}. \\

Hence, by Monotone Convergence Theorem
\begin{align*} 
  \explaintermbrace{$=\liminf_{n \to \infty} \dint g_n \dmu$}{\dlim_{n \to \infty} \dint g_n \dmu} \stackexplain{(MCT)}{=} \dint \dlim_{n \to \infty} g_n \dmu  && \tinycircled{1} 
\end{align*}
So
\[ \liminf_{n \to \infty} \dint f_n \dmu \stackexplain{(monotonicity, $f_n \geq g_n$)}{\geq}  \liminf_{n \to \infty} \dint g_n \dmu \stackexplain{\tinycircled{1}}{=} \dint \lim_{n \to \infty} g_n \dmu \stackexplain{(def. $g_n$)}{=} \dint \liminf_{n \to \infty} f_n \dmu \]
\end{proof}

\begin{remark}{\remarktitle{Expanded view of Fatou's Lemma}}
We can interpret Fatou's lemma as \textit{integrals of asymptotics give more extreme values than asymptotics of integrals}.

To see this, first note that a parallel argument gives 
...
\begin{theorem}{\remarktitle{Integrals as measures}}
...
\begin{remark}{\remarktitle{Change of measure}} Rudin \cite{walter1987real} points out an important implication of Theorem \ref{thm:integrals_as_measures}: if $h \geq 0$, and so $\lambda$ (as defined as in the theorem statement) is a measure, then
\begin{align*}
\dint_\Omega g \wrt{\lambda} = \dint_\Omega g h \dmu
...
The Radon-Nikodym theorem (to be covered later in the document) provides an important converse:  instead of obtaining new measure $\lambda$ given original measure $\mu$ and function $h$, we will be given two measures $\lambda$ and $\mu$, and will obtain the \textit{Radon-Nikdoym derivative} $h$.
...
\begin{theorem}{\remarktitle{Additivity theorem}}
...
\begin{alphabate}
\item If $h_1, h_2, ...$ are non-negative Borel measurable, 
...
\begin{proof}
\begin{alphabate}
\item $\ds\sum_{n=1}^N h_n \uparrow \ds\sum_{n=1}^\infty h_n$, so we apply Monotone Convergence Theorem (MCT)
\[ \explaintermbrace{ $\stackexplain{(additivity)}{=} \dsum_{n=1}^\infty \dint h_n \dmu$}{\dlim_{N \to \infty} \dint \dsum_{n=1}^N h_n \dmu} \stackexplain{(MCT)}{=} \dint \dsum_{n=1}^\infty h_n \dmu \]
...
\begin{definition}{\remarktitle{Almost everywhere}}
...
\begin{theorem}{\remarktitle{Almost everywhere}}
