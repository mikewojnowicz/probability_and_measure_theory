...
Below we show some properties of Borel sets that hold under Lesbesgue measure.  How can we accomplish this?  After all, the Borel sets are rather abstract, and also contain many types of members (e.g., proper open intervals, proper closed intervals, singletons, bizarre sets like the Cantor set, etc.).

To prove that such properties hold, we can take a standard tact: use the Good Sets strategy, in conjunction with the Monotone Class Theorem,  to show that the Borel sets have some property (see Remark \ref{rk:monotone_class_theorem_for_executing_good_sets_strategy}).  Using this strategy, we can show that a property holds for \textit{all} Borel sets if we can just show that the property holds for some field generating the Borel sets (e.g., $\F_0 := \set{\text{disjoint unions of } (a,b], a,b \in \R^n }$) -- a much more tangible object to work with.
...
Note that the strategy of Remark \ref{rk:monotone_class_theorem_for_executing_good_sets_strategy_with_borel_sets} is simply the strategy from Remark \ref{rk:monotone_class_theorem_for_executing_good_sets_strategy} in the special case where the $\sigma$-field of interest is the Borel sets (and where we take the field generating them to be $\F_0$).
...
\subsection{A set that is not Lesbesgue measurable\footnote{The presentation here is a variation on both Problem 6 in Section 1.4 of 
\cite{ash2000probability} and Proposition 1.2.6 in \cite{rosenthal2006first}.}}

\begin{proposition}
Call $x,y \in \R$ equivalent iff $x-y \in \mathbb{Q}$.\footnote{So, for example, some equivalence classes are:
\begin{itemize}
\item $e \sim e + \frac{1}{10} \sim e - \frac{1}{10} \sim e+\frac{50}{3} + ...$
\item  $\pi \sim \pi + \frac{1}{10} \sim \pi - \frac{1}{10} \sim \pi+\frac{50}{3} + ...$
\item $1 \sim 1 + \frac{1}{10} \sim 1 - \frac{1}{10} \sim 1 +\frac{50}{3} + ...$
\end{itemize}
...
\item First, we show that we can partition the reals as $\R = \bigcupdot_{q \in \mathbb{Q}} (q+A)$.  
	\begin{itemize}
		\item \textit{(disjointedness)} Suppose $x \in q+A$ and $x \in r+A$ for some $r,q \in \mathbb{Q}, r \neq q$.  Then $\exists\, a_1,a_2 \in A : q + a_1 = r + a_2 \implies a_1 - a_2 = r - q$.  Since $r \neq q, a_1 - a_2 \neq 0 \implies a_1 \neq a_2$.  But since $r-q \in \mathbb{Q}$, $a_1$ and $a_2$ are in the same equivalence class, which contradicts how $A$ was constructed.
	...
	\end{itemize}
\item Now we argue
\[ [0,1] = \R \cap [0,1] \stackrel{\text{part (a)}}{=} \bigg( \bigcupdot_{q \in \mathbb{Q}} (q+A)\bigg) \cap [0,1] = \bigcupdot_{q \in \mathbb{Q}} \bigg( (q+A) \cap [0,1] \bigg)  \]
and so
\begin{align*}
 1 = \mu[0,1] \stackrel{\text{countable additivity, above}}{=}  \ds\sum_{q \in \mathbb{Q}} \mu\bigg( (q + A) \cap [0,1] \bigg)  \stackrel{\text{translation invariance}}{=} \ds\sum_{q \in \mathbb{Q}}  \mu\bigg(  A \cap [0,1] \bigg)  
 \labelit \label{eqn:contradiction_to_lesbesgue_measurability_for_all_subsets}	
\end{align*}

but this is a contradiction; the RHS of this equation is a countable sum of a constant, and so can only take on values $-\infty, 0,$ or $\infty$.
\end{alphabate}
...
\item finiteness on bounded intervals (note that the argument in part (b) would go through if we intersected the reals with any bounded interval $[a,b]$).
\end{itemize}
