...
\red{TODO: Fill in, using the arguments of Folland. Be sure to give the reader a pointer that we're using \cite[pp.68]{folland1999real}.  Give the definitional construction along with the simplified construction.}



\subsection{Extended product measure theorem}

\red{TODO: Add narrative about how we get an alternate view on the product measure, or even on a new thing (which seems like it will be useful for conditional probabilities) which we currently call an "extended product measure".}
...
for all $A \in \M, B \in \N$.  Moreover, $\lambda$ is $\sigma$-finite on $\M \otimes \N$, and is a probability measure if $\mu$ and $\nu$ are.  The measure $\lambda$ is called the product of $\mu$ and $\nu$, and is written $\lambda = \mu \times \nu$.
\label{cor:classical_product_measure_theorem}
\end{corollary}

\begin{proof}
In Theorem~\ref{thm:extended_product_measure_theorem}, take $\nu(x, \cdot) = \nu$ for all $x \in X$.  The second formula for $\lambda$ is obtained by interchanging $\mu$ and $\nu$.	
\end{proof}
...
The classical Fubini-Tonelli Theorem says that for $\sigma$-finite measure spaces, integrating against product measure is the same as iterated integration in either order. 
...
\begin{enumerate}
\item \textit{Step 1: $f$ is an indicator function}.   Here, Tonelli's theorem reduces to the classical product measure theorem {\tiny  [$\lambda(E) := \int \nu(E_x) \mu(dx) = \int \mu(E^y) \nu(dy) $; see Cor.~\ref{cor:classical_product_measure_theorem}.  Note that when integrating $f(x,y)$ against one variable, the function becomes a section.  So when integrating $f(x,y)$ against a measure on $y$, it becomes $f_x(y)$.]}  
...
%
\begin{align*}
\int \int \indicate{D} \wrt{\mu} \wrt{\nu} & = \int \int \indicate{ \set{(x,y) \, : \, y \in [0,1], x=y}} \wrt{\mu(x)} \wrt{\nu(y)} \\
...
%
\begin{align*}
\int \int \indicate{D}  \wrt{\nu} \wrt{\mu} & = \int \int \indicate{ \set{(x,y) \, : \, x \in [0,1], y=x}} \wrt{\nu(y)} \wrt{\mu(x)}  \\
& = \int_{[0,1]} \int_{\set{y: y=x}} 1  \wrt{\nu(y)} \wrt{\mu(x)} \\
& = \int_{[0,1]} \nu(\set{y: y=x}) \wrt{\mu(x)} \\
&= \int_{[0,1]} 1 \wrt{\mu(x)} = \mu([0,1]) = 1
\end{align*}

\item Now we show $\int \int \indicate{D} \wrt{(\mu \times \nu)} = \infty$. By the definition of integrals of simple functions, we have $\int \int \indicate{D} \wrt{(\mu \times \nu)} = \mu \times \nu(D)$.  So applying the (computational) definition of product measure \red{TODO: Need to add this in}, we want to show 
\begin{align}
\mu \times \nu(D): = \inf \bigg\{ \ds\sum_{n=1}^\infty \mu(A_n) \nu(B_n) : \cup_{n=1}^\infty (A_n \times B_n) \supseteq D, \; A_n \in \B(\R), B_n \subseteq [0,1] \bigg\} = \infty 
...
\begin{align*}
\tinytext{$\tinycircled{1}$} \implies & \mu([0,1]) \stackexplain{monotonicity}{\leq} \mu(\cup_{n=1}^\infty A_n \cap B_n) \stackexplain{subadditivity}{\leq} \ds\sum_{n=1}^\infty \mu(A_n \cap B_n) 	&& \\
