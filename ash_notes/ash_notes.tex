...
For another example, this time in two dimensions, suppose we want to find a mountain's volume (above sea level).
...
Now we show that the Dirichlet function does not have a Reimann integral. Recall that the Reimann integral $\int_{a}^b f(x) \, dx$, when it exists,  is defined only for real-valued functions  $f$ whose domain is a compact space $[a,b] \subset \R$. For more information, see Section \ref{sec:some_info_relevant_to_reimann_integrals}.

\begin{remark}{\remarktitle{The Dirichlet function does not have a Reimann integral.}}	
Let $f = I_{\Q} : [a,b] \to \R$ be the Dirichlet function on $[a,b] \subset \R$.  We consider an arbitrary \textit{partition} $P$ of $[a,b]$ into a collection of $n$ subintervals $[x_{k-1}, x_k]$ where $a=x_0 < x_1 < ... < x_n = b$.  Now by Proposition \ref{prop:reimann_integrable_iff_oscillation_goes_to_zero_as_partition_gets_finer}, a function on $[a,b]$ is Reimann integrable iff $\text{Osc}(f,P) \to 0$ as the maximum interval length of a partition $P$ goes to 0.  But for any partition $P$, any subinterval $[x_{k-1}, x_k]$ will contain at least one rational number and at least one irrational number, and so
\begin{align*}
