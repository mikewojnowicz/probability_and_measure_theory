...
\begingroup
\makeatletter
\apptocmd{\thedefinition}{\unless\ifx\protect\@unexpandable@protect\protect\footnote{Note that in $\S$2.7.1-2.7.3, \cite{ash2000probability} uses $B_n$ for cylinder and $B^n$ for base.  However, I find the superscript/subscript notation confusing.  Therefore, I use a different notation, which actually comes from $\S$2.7.4-2.7.5, the uncountable product section of \cite{ash2000probability}. The notation $B^n(v)$ for a cylinder can be interpreted as the set $B^n$ located at coordinates $v=(t_1, \hdots, t_n)$.}\fi}{}{}
\makeatother

\begin{definition}
Given $v:=(t_1, \hdots, t_n)$, a finite subset of $\set{1,2,\hdots}$, and a set $B^n \in \Omega_v := \prod_{i=1}^n \Omega_{t_i}$, we define a \textbf{cylinder} as
\begin{align}
 \explaintermbrace{cylinder}{B^n(v)} := \set{\omega \in \Omega : (\omega_{t_1}, \hdots \omega_{t_n}) \in \explaintermbrace{base}{B^n}}  
\label{eqn:cylinder_in_sequence_space}	
\end{align}
If $B^n \in \F_v := \prod_{i=1}^n \F_{t_i}$, we call this a \textbf{measurable cylinder}.
\end{definition}
\endgroup

\begin{definition}
A cylinder whose base is a rectangle with $n$ sides (i.e. $B^n = A_i \times \hdots \times A_n$ with $A_i \in \Omega_{t_i}$ for all $i$) is called a \textbf{rectangle}.   If $A_i \in \F_{t_i}$ for each $i$, the cylinder is called a \textbf{measurable rectangle}. 
\end{definition}
...
Let $\Omega_i = \R$ for all $i$ (written $\Omega = \R^\mathbb{N}$), and let $B^2$ be the base given in Fig.~\ref{fig:cylinder_base_in_2_coordinates}. A cylinder can be defined for any choice of $v=(t_1, t_2)$ by
\begin{align}
B^2(v) := \set{ \omega \in \Omega : (\omega_{t_1}, \omega_{t_2}) \in B^2 }  
\end{align}
For instance, $(1.5, 1.5, \omega_3, \omega_4, \hdots) \in B^2(1,2)$, and  $(1.5, \omega_2, 1.5, \omega_4, \omega_5, \hdots) \in B^2(2,4)$.

The projection of the cylinder $B^2(t_1, t_2)$ onto $(\omega_{t_1}, \omega_{t_2}, \omega_{t_3})$ is given in Fig.~\ref{fig:cylinder_projected_to_first_three_coordinates}.  
...
For example, given a base $B^3$, we can write:
\[ \explaintermbrace{base}{B^3}(1,2,3) = [\explaintermbrace{base}{B^3 \times \Omega_4}] (1,2,3,4) \]
since
\begin{align*}
B^3(1,2,3)&=\set{\omega \in \Omega : (\omega_1, \omega_2, \omega_3) \in B^3} \\
&=\set{\omega \in \Omega : (\omega_1, \omega_2, \omega_3) \in B^3, \omega_4 \in \Omega_4} \\
&=\set{\omega \in \Omega : (\omega_1, \omega_2, \omega_3, \omega_4) \in B^3 \times \Omega_4} \\	
...
\red{TODO [WIP]: I think the preceding exposition and notation implicitly assumes each set $\Omega_i$ is identical; I seem to be assuming that the base $B^n$ can be put along\textit{any} coordinates by choice of $v$, but this clearly isn't true (e.g. suppose $\Omega_i=\R$ if $i$ odd and $\Omega_i=\mathbb{N}$ if $i$ even).  I need to review this and re-notate.}
...
\item if $B^n(v) \in \C$, then $B^n(v)^c$ is a finite disjoint union of members of $\C$? \greencheck  In fact, if $B^n(v) \in \C$, then $B^n(v)^c \in \C$.  To see this, observe that given a base $B^n \in \F_v$, we have
\begin{align*}
(B^n(v))^c &= \set{\omega \in \Omega : (\omega_{t_1}, \hdots \omega_{t_n}) \in B^n}^c \\
&= \set{\omega \in \Omega : (\omega_{t_1}, \hdots \omega_{t_n}) \in \explaintermbrace{$\in \F_v$, since $\F_v$ is a $\sigma$-field}{(B^n)^c}}
\end{align*}
\item if $B^n(v),A^m(w) \in \C$ then $B^n(v) \cap A^m(w) \in \C$? \greencheck  By Remark~\ref{rk:cylinder_can_always_be_regarded_as_having_a_higher_dimensional_base}, we can assume that $m=n$ and $v=w$, so it remains to show that if $B^n(v),A^n(v) \in \C$ then $B^n(v) \cap A^n(v) \in \C$.  This holds because 
\[ B^n(v) \cap A^n(v) = \set{\omega \in \Omega : (\omega_{t_1}, \hdots \omega_{t_n}) \in \explaintermbrace{$\in \F_v$, since $\F_v$ is a $\sigma$-field}{B^n \cap A^n}}
 \]
...
\red{TODO: Add a meta-commentary about how we're doing something similar to what we did when constructing the Borel sets -- define an elementary family, extend to a field, define a measure on the field, use Caratheodory to extend to a measure on a sigma field.} 
