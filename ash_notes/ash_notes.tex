...
\begin{theorem}{\textbf{(\Caratheodory~Extension Theorem)}} Let $\mu$ be a measure on a field $\F_0$ of subsets of $\Omega$, and assume that $\mu$ is $\sigma$-finite on $\F_0$, so that $\Omega$ can be decomposed as $\cup_{n=1}^\infty A_n$ where $A_n \in \F_0$ and $\mu(A_n) < \infty$ for all $n$.  Then $\mu$ has a unique extension to a measure on $\F := \sigma(\F_0)$, the minimal $\sigma$-field over $\F_0$. 
...
%
\begin{align}
\mu(A) = \inf \bigg\{ \ds\sum_{j=1}^\infty \mu_0(A) : A_j \in \F_0, \bigcup_{j=1}^\infty A_j \supset A  \bigg\} \quad \quad \quad \forall A \in \F
...
\subsubsection{Construction of the product measure}

Let $C \in F_0$ be a disjoint union of rectangles $A_1 \times B_1, A_2 \times B_2, \hdots A_n \times B_n$.  {\tiny  (By Remark Rk.~\ref{rk:the_collection_of_finite_disjoint__unions_of_rectangles_is_a_field},  $F_0$ is a field. ) }
...
\item \textit{Step 3: $f$ is a general non-negative measurable function}.  We can find a sequence of simple functions $\set{f_n} \uparrow f$ by Prop.~\ref{prop:there_is_a_sequence_of_simple_fucntions_that_increases_to_any_non_negative_borel_measurable_function}.   We now show two things:
	...
	\end{itemize}
\end{enumerate}
\red{TODO: Finish proof of Fubini part of Fubini-Tonelli}
\end{proof}
...
\begin{figure}[H]
\centering 
\includegraphics[width=.4\textwidth, angle=90]{images/lebesgue_counting_measure_on_unit_square_diagonal}	
\end{figure}
