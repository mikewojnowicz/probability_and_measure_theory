...
Let $\Omega = \set{1,2,3, ...}$, $\F=2^\Omega$ (i.e. all subsets of $\Omega$), and $\mu$ be the counting measure. A real-valued function $f$ on $\Omega$ can be written as a sequence of real numbers; we write $f=\set{a_n}, n=1,2, \hdots$.  We will show that an integral on this space is really a sum:
%
\begin{align}
...
We know how to integrate each $f_k$, since we by the definition of integrals of simple functions, we have
%
\begin{align}
...
\subsection{$L^p$ spaces as normed vector spaces}


\begin{definition}{(\textbf{$L^p$ space}.)} If $p>0$, we define the space $L^p = L^p(\Omega, \F, \mu)$ as the collection of complex-valued Borel measurable functions $f$ such that $\int_\Omega |f|^p \dmu < \infty$.
\end{definition}


We note that $L^p$ is a \textit{vector space} (see Sec.~\ref{sec:vector_space}) for $1 \leq p < \infty$. We check:
\begin{itemize}
...
\item \textit{Closure under addition?} \greencheck.  Given by Minkowski's inequality below.  	
\end{itemize}
...
Functions in $L^p$ can be approximated nicely.  We point out two basic approximation theorems.

\begin{itemize}
\item Finite valued simple functions are dense in $L^p$ (see Theorem ~\ref{thm:finite_valued_simple_functions_are_dense_in_Lp}). Here  $L^p = L^p((\Omega, \F, \mu))$, where $(\Omega, \F, \mu)$ is any measure space.

\item Continuous functions are dense in $L^p(\R^n, \B(\R^n), \mu)$, where $\mu$ is a Lebesgue-Stieltjes measure.  (See \cite[Thm~2.4.14]{ash2000probability}.)
\end{itemize}
...
\end{align*}
\item \textbf{Converge in $L^p$}, written $f_n \stackrel{L^p}{\to} f$, if	
...
In the Figures below, we summarize the relationship between modes of convergence using a charts from \cite[pp.75]{bartle2014elements}. In each chart, a solid line means that convergence in the mode at the tail of the arrow implies convergence in the mode at the head.  
...
\subsubsection{Almost uniform convergence and convergence in measure} 

Now we show that almost uniform convergence is stronger than convergence in measure.
\begin{theorem}
\[ f_n \convergenceAU f \implies f_n \convergenceInMu f \]	
\label{thm:convergence_AU_implies_convergence_in_measure}
\end{theorem}

\begin{proof}
We take the definition of $\convergenceAU$ in quantified form \eqref{eqn:def_of_convergence_almost_uniformly_in_quantified_form} 
\begin{align*}
...
& |f_n(\omega) - f(\omega) | < \epsilon 
\end{align*}
and remove the explicit reference to the set $A$. In particular, for sufficiently large $n$, we have
\begin{align*} 
...
As noted in Sec.~\ref{sec:overview_of_relations_between_modes_of_convergence}, finite measure spaces have two implications that doesn't exist for general measure spaces:  $f_n \convergenceAE f \implies f_n \convergenceAU f, \convergenceInMu f$.  We obtain the former implication via Egoroff's Theorem, and then the latter follows immediately from Thm.~\ref{thm:convergence_AU_implies_convergence_in_measure}.
...
Recall that $f_n \to f$ uniformly on $A$ means 
\begin{align*}
&\forall \epsilon <0, \; \exists N_\epsilon : \;\forall n \geq N_\epsilon, x \in A\\
& |f_n(x) - f(x) | < \epsilon 
\end{align*}
By the density of the rationals, an equivalent condition is
\begin{align*}
