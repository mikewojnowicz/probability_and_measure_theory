...
\end{remark}
 ...
 \begin{proposition}
 Let $\mu$ be a Lesbesgue-Stieltjes measure, and let $F$ be its associated distribution function.    Let $F(x^-) = \lim_{y \to x^-} F(y)$. Then 
...
\labelit \label{eqn:measure_as_distribution_function_difference_for_motivating_LS_in_Rn}
\end{align*}
does \textit{not} hold! To see this, for now let us construct a distribution function $F : \R^n \to \R$ via 
\[  F(a) = \mu(-\infty, a] = \mu(\omega \in \R^n : \omega_1 \leq x_1, ..., \omega_n \leq x_n)\]
...
For example, in Figure \ref{fig:distribution_functions_in_Rn}, we see that if $(a,b] = I_1 \times I_2 = (a_1,b_1] \times (a_2, b_2]$, then 
\begin{align*}
...
If $\mu$ is a finite measure on $\B(\R^n)$, we can generalize \eqref{eqn:measure_of_rsc_interval_in_R2} for measuring r.s.c. intervals to $\R^n$.

\begin{theorem}
Let $\mu$ is a finite measure on $\B(\R^n)$.  Then 
\begin{alphabate}
\item We have 	
	\begin{align*}
	...
	\end{align*}
\item We have
\[\Delta_{(a,b)} F = \sum_{v \in V(a,b]} (-1)^{\# \text{ of a's in v}} \; F(v) \]
where $V(a,b]$ are the vertices of $(a,b]$ (see Definition \ref{def:vertices_of_rsc_interval_in_Rn}). 
\end{alphabate}
\label{eqn:measure_of_rsc_interval_in_Rn_via_distribution_function}
\end{theorem}
...
\begin{remark}
Note from the proof of Theorem \ref{eqn:measure_of_rsc_interval_in_Rn_via_distribution_function} part (a) that the application of the $n$th difference operator restricts the set being measured to the bounds given in the $n$th dimension.  See Figure \ref{fig:applying_the_difference_operator_to_distribution_functions_in_R2}.
...
Due to Theorem \ref{eqn:measure_of_rsc_interval_in_Rn_via_distribution_function} part (a), we must alter our notion of \textit{increasing} when defining distribution functions on $\R^n$.  
...
\begin{definition}
 ...
 However, such a condition would be insufficient to describe a distribution function in $\R^n.$ For an example of a distribution function that is right continuous and increasing in this sense, but which assigns negative measure to intervals, see pp. 6-7 of \cite{durrett2010probability}.
 \end{remark}
