...
A special case is where each $F_i(x_i) = x_i$, in which case each $F_i$ is Lesbesgue measure on $\B(\R)$.  Then we have
\[ F(x_1,...,x_n) = x_1x_2 \cdots x_n\]
and so 
\[ \mu(a,b] = \Delta_{(a,b]} F = \ds\prod_{i=1}^n (b_i-a_i) \]
This $\mu$ is \textit{Lesbesgue measure} on $\B(\R^n)$. The measure of any rectangular box is its volume. 
