...
\item Measure theory provides nice limit theorems, e.g. clarifying when one can interchange integrals and limits (such as derivatives). 
\item Integrating against arbitrary measure provides the necessary abstraction to integrate over spaces more general than the reals. 
...
The Cantor set can quite easily be defined in terms of a countable intersections and unions of closed sets. In each step you "remove the middle third of each part". Explicitly,
...
\subsection{Intuition\footnote{Here we borrow freely from some sections of Wikipedia.}}
...
\begin{definition}
If $h : \Omega_1 \to \Omega_2$, $h$ is \textit{measurable} relative to the $\sigma$-fields $\F_j$ of subsets of $\Omega_j$, $j=1,2$, iff $h^{-1}(A) \in F_1$ for all $A \in \F_2$.  We denote such a function as $h : (\Omega_1, \F_1) \to (\Omega_2, \F_2)$. 
...
\begin{definition}
A \textit{Borel measurable function} is a function $h : (\Omega_1, \F_1) \to (\R^n, \B(\R^n))$ or  $h : (\Omega_1, \F_1) \to (\overline{\R}^n, \B(\overline{\R}^n))$. 
...
\begin{remark}
Thanks to Claim \ref{claim:computational_def_measurability}, to show that a function $h$ is Borel measurable, we simply need to show that $h^{-1}(B) \in \F_1$ for $B \in \C$ for any collection $\C$ that generates the Borel sets; for instance it suffices to show
\begin{itemize}
...
%
%\begin{example}
%Let $A$ be a Borel subset of $\R$ and let $I_A$ be the indicator of $A$; that is $I_A(\omega)=1$ for $\omega \in A$ and $0$ for $\omega \not\in A$. 	
%\end{example}
