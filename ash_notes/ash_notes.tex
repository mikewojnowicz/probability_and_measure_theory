...
\subsection{Extension and approximation}
...
\item We need to show that if $A_1 \cup S_1 = A_2 \cup S_2$, then $\mu(A_1 \cup S_1) = \mu(A_2 \cup S_2)$, or more simply that $\mu(A_1)=\mu(A_2)$.
...
\begin{question}
Remark \ref{rk:completion_of_measure_space_is_well_defined} could be augmented to explicitly argue that countable additivity holds in $\F_\mu$. How to show that?
\end{question}
