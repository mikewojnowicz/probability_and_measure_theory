...
\red{TODO: Fill in}
% Add proof
% Add definition of weak convergence.

\begin{theorem}\textnormal{(Weak convergence as convergence of distribution functions at points of continuity.)} Let $\mu, \mu_1, \mu_2, \hdots$ be finite measures on $\B(\R)$, with corresponding distribution functions $F, F_1, F_2, \hdots$.\footnote{Note that any finite measure is a Lebesgue-Stieltjes measure, and hence has a corresponding distribution function.} The following are equivalent:
...
\begin{itemize}
\item $\boxed{(a) \implies (b)}$.	\red{TODO: Fill in}
...
\begin{enumerate}
	...
	\begin{enumerate}
	\item[2i.)] Thus, we can approximate any open interval $I_k=(a_k,b_k)$ arbitrarily well by a right semi-closed subinterval $I_k'=(a_k',b_k']$ whose endpoints are continuity points of $F$.  \red{TODO: Add picture}
	\end{enumerate}
	\item[3.)] $\mu_n(I_k') \to \mu(I_k')$ by (b).
	\item[4.)] Now we prove weak convergence (characterization  Thm.~\ref{thm:weak_convergence} (d))  by
	\begin{align*}
	...
	&\stackexplain{(3.)}{=} \sum_{k=1}^\infty \mu(I_k') \\
