...
\begin{example-for-statisticians}{\remarktitle{Jeffreys prior for observation noise in linear regression}}
...
\begingroup
\makeatletter
\apptocmd{\thedefinition}{\unless\ifx\protect\@unexpandable@protect\protect\footnote{Note that in $\S$2.7.1-2.7.3, \cite{ash2000probability} uses $B_n$ for cylinder and $B_n$ for base.  However, I find the superscript/subscript notation confusing.  Therefore, I use a different notation, which actually comes from $\S$2.7.4-2.7.5, the uncountable product section of \cite{ash2000probability}. The notation $B_n(v)$ for a cylinder can be interpreted as the set $B_n$ located at coordinates $v=(t_1, \hdots, t_n)$.}\fi}{}{}
\makeatother

\begin{definition}
Given $v:=(t_1, \hdots, t_n)$, a finite subset of $\set{1,2,\hdots}$, and a set $B_n \in \Omega_v := \prod_{i=1}^n \Omega_{t_i}$, we define a \textbf{cylinder} as
\begin{align}
 \explaintermbrace{cylinder}{B_n(v)} := \set{\omega \in \Omega : (\omega_{t_1}, \hdots \omega_{t_n}) \in \explaintermbrace{base}{B_n}}  
\label{eqn:cylinder_in_sequence_space}	
\end{align}
If $B_n \in \F_v := \prod_{i=1}^n \F_{t_i}$, we call this a \textbf{measurable cylinder}.
\end{definition}
\endgroup

\begin{definition}
A cylinder whose base is a rectangle with $n$ sides (i.e. $B_n = A_i \times \hdots \times A_n$ with $A_i \in \Omega_{t_i}$ for all $i$) is called a \textbf{rectangle}.   If $A_i \in \F_{t_i}$ for each $i$, the cylinder is called a \textbf{measurable rectangle}. 
\end{definition}
...
Let $\Omega_i = \R$ for all $i$ (written $\Omega = \R^\mathbb{N}$), and let $B_2$ be the base given in Fig.~\ref{fig:cylinder_base_in_2_coordinates}. A cylinder can be defined for any choice of $v=(t_1, t_2)$ by
\begin{align}
B_2(v) := \set{ \omega \in \Omega : (\omega_{t_1}, \omega_{t_2}) \in B_2 }  
\end{align}
For instance, $(1.5, 1.5, \omega_3, \omega_4, \hdots) \in B_2(1,2)$, and  $(1.5, \omega_2, 1.5, \omega_4, \omega_5, \hdots) \in B_2(2,4)$.

The projection of the cylinder $B_2(t_1, t_2)$ onto $(\omega_{t_1}, \omega_{t_2}, \omega_{t_3})$ is given in Fig.~\ref{fig:cylinder_projected_to_first_three_coordinates}.  
...
For example, given a base $B_3$, we can write:
\[ \explaintermbrace{base}{B_3}(1,2,3) = [\explaintermbrace{base}{B_3 \times \Omega_4}] (1,2,3,4) \]
since
\begin{align*}
B_3(1,2,3)&=\set{\omega \in \Omega : (\omega_1, \omega_2, \omega_3) \in B_3} \\
&=\set{\omega \in \Omega : (\omega_1, \omega_2, \omega_3) \in B_3, \omega_4 \in \Omega_4} \\
&=\set{\omega \in \Omega : (\omega_1, \omega_2, \omega_3, \omega_4) \in B_3 \times \Omega_4} \\	
&= [B_3 \times \Omega_4](1,2,3,4)
\end{align*}
...
\item if $B_n(v) \in \C$, then $B_n(v)^c$ is a finite disjoint union of members of $\C$? \greencheck  In fact, if $B_n(v) \in \C$, then $B_n(v)^c \in \C$.  To see this, observe that given a base $B_n \in \F_v$, we have
\begin{align*}
(B_n(v))^c &= \set{\omega \in \Omega : (\omega_{t_1}, \hdots \omega_{t_n}) \in B_n}^c \\
&= \set{\omega \in \Omega : (\omega_{t_1}, \hdots \omega_{t_n}) \in \explaintermbrace{$\in \F_v$, since $\F_v$ is a $\sigma$-field}{(B_n)^c}}
\end{align*}
\item if $B_n(v),A_m(w) \in \C$ then $B_n(v) \cap A_m(w) \in \C$? \greencheck  By Remark~\ref{rk:cylinder_can_always_be_regarded_as_having_a_higher_dimensional_base}, we can assume that $m=n$ and $v=w$, so it remains to show that if $B_n(v),A_n(v) \in \C$ then $B_n(v) \cap A_n(v) \in \C$.  This holds because 
\[ B_n(v) \cap A_n(v) = \set{\omega \in \Omega : (\omega_{t_1}, \hdots \omega_{t_n}) \in \explaintermbrace{$\in \F_v$, since $\F_v$ is a $\sigma$-field}{B_n \cap A_n}}
 \]
...
Due to Remark~\ref{rk:cylinder_can_always_be_regarded_as_having_a_higher_dimensional_base}, we will assume WLOG in the Theorem that $v=(1,\hdots,n)$. 
...
\begin{align}
P_n(B_n) &= \int_{\Omega_1} P_1(\wrt{\omega_1}) \int_{\Omega_2} P_2(\omega_1; \wrt{\omega_2}) \cdots \int_{\Omega_{n-1}} P_{n-1}(\omega_1, \hdots \omega_{n-2}; \wrt{\omega_{n-1}})  \\&\quad \int_{\Omega_{n}} P_{n}(\omega_1, \hdots \omega_{n-1}; \wrt{\omega_{n}}) \;\indicate{(\omega_1,\hdots, \omega_n) \in B_n}  
\label{eqn:product_measure_in_n_coordinates}
\end{align}	
Note that $P_n$ is a probability measure by \cite[Thm.~2.6.7]{ash2000probability}.

Then there is a unique probability measure $P$ on $\F$ such that for all $n$, $P$ agrees with $P_n$ on $n$-dimensional cylinders; that is $P\set{\omega \in \Omega : (\omega_1, \hdots, \omega_n) \in B_n} = P_n(B_n)$ for all $n=1,2,\hdots$ and $B_n \in \prod_{j=1}^n \F_j$. 
\end{theorem}
...
Suppose $B_n(1:n)=C_m(1:m)$ for $m<n$.\footnote{Recall from the definition of cylinders that 
\begin{align*}
 B_n(1:n) &= \set{\omega \in \Omega : (\omega_1, \hdots, \omega_n) \in B_n}, \quad B_n \in \prod_{i=1}^n \F_i \\
 C_m(1:m) &= \set{\omega \in \Omega : (\omega_1, \hdots, \omega_n) \in  C_m}, \quad  C_m \in \prod_{i=1}^m \F_i 
\end{align*}
} Then we can relate the bases of the cylinders by $B_n = C_m \times \Omega_{m+1} \times \cdots \times \Omega_n$.

We need to show that the probability mass assigned to the cylinder is invariant to representation, i.e. $P(B_n(1:n))=P(C_m(1:m)) \stackexplain{(def P)}{\iff} P_n(B_n)=P_m(C_m)$. 
...
\begin{align*}
B_{n+1}(:n+1) & \subset B_n(:n) && \tinytext{decreasing seq of cylinders} \\
...
Repeat this inductively (See \cite{ash2000probability}) to obtain points $(\omega_1', \omega_2', \hdots)$ such that for each $n$, $(\omega_1', \hdots, \omega_n') \in B_n$.  Hence $(\omega_1', \omega_2', \hdots) \in \cap_{n=1}^\infty B_n(:n) = \emptyset$, a contradiction. 
\end{enumerate}
