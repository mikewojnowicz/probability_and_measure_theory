...
So let $f$ be a arbitrary non-negative Borel function.   We want to show that if $\lambda(B) = \int_B f \wrt{\mu}$,  $B = \bigcupdot_{n=1}^\infty B_n$, then $\lambda(B) = \ds\sum_{n=1}^\infty \lambda(B_n)$.
...
\begin{align*}
\ds\int_{B_n} s_n \wrt{\mu} \geq \ds\int_{B_n} f_n \wrt{\mu} - \frac{\epsilon}{N} \quad \text{ for all } n  && \tinycircled{2} 
\end{align*}
...
&= \ds\sum_{n=1}^N \ds\int_{B_n} f_n \wrt{\mu} - \epsilon && \tinytext{see \tinycircled{4}} \\
...
\begin{remark}{\remarktitle{Change of measure}} Rudin \cite{walter1987real} points out an important implication of Theorem \ref{thm:integrals_as_measures}: if $f \geq 0$, and so $\lambda$ (as defined as in the theorem statement) is a measure, then
\begin{align*}
