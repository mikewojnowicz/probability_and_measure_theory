...
Below we show that the Radon-Nikodym derivative satisfies a chain rule. 

\begin{proposition}{\remarktitle{A chain rule for Radon-Nikodym derivatives.}}
Suppose that $\nu$ is a $\sigma$-finite signed measure and $\mu, \lambda$ are $\sigma$-finite measures on $(\Omega, \F)$ such that $\nu \ll \mu$ and $\mu \ll \lambda$.
Then 
\begin{alphabate}
\item If $g$ is integrable, then $g(d\nu/d\mu)$ is integrable, and 
\[ \int g \wrt{\nu} = \int g \frac{d\nu}{d\mu} \wrt{\mu} \]
\item We have $\nu \ll \lambda$, and 
...
Since \eqref{eqn:RN_chain_rule_RHS}  and \eqref{eqn:RN_chain_rule_LHS} both hold for all $A \in \F$, then by \cite{folland1999real} Prop. 2.23 \red{(TODO: But how do we know from this that $\frac{d\nu}{d\mu} \indicate{A}$ is integrable!?)}, 
\[ \df{d\nu}{d\lambda} = \df{d\nu}{d\mu}  \df{d\mu}{d\lambda} \quad \lambda-\text{a.e.} \]	
