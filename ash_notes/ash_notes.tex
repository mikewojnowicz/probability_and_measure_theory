...
\begin{theorem}
Let $h$ be a Borel measurable function such that $\int_\Omega h \wrt{\mu}$ exists.  Define $\lambda(B) = \int_B h \wrt{\mu}$, $B \in \F$.  The $\lambda$ is countably additive on $\F$; thus if $h \geq 0$, $\lambda$ is a measure.	
\end{theorem}
