...
\begin{proposition}{\remarktitle{All sections of a measurable set are measurable.}} If $(X,\M)$ and $(Y,\N)$ are measurable spaces, and $E \in \M \otimes \N$, then $E_x \in \M$ for every $x \in X$ and $E_y \in \N$ for every $y \in Y$. 
\label{prop:sections_of_a_measurable_set_are_measurable}
\end{proposition}

\begin{proof}
We apply the ``Good sets strategy" (Sec.~\ref{sec:good_sets_strategy}).   Let
\[  \G := \{ E \subset X \times Y : E_x \in \N \text{ for all} \; x \in X \quad \text{and} \quad E_y \in \M \text{ for all} \; y \in Y \}. \]
We have
\begin{itemize}
