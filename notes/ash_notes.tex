...
These notes are geared towards statisticians, computer scientists, engineers, and others who are interested in measure theory in the service of data science (specifically, statistics or statistical machine learning).  The primary reference is \citet{ash2000probability}.   The book is wonderful for statistical machine learning – it is rigorous,  but also accessible (prerequisites are undergrad-level real analysis and mathematical probability).  Most importantly, it is structured to build towards the kinds of applications in probability that we care about.   (A point of contrast would be a book like that of Stein and Shakarchi, which tends to dwell heavily on things that are of higher interest to pure mathematicians – Cantor sets and fractals, etc.) 
 ...
 Other useful references are \citet{folland1999real} and \citet{rudin1987real} for filling in depth. \citet{durrett2010probability} has excellent examples and high-level overviews, but in my view does not provide enough detail to become fluent in the topic. \citet{axler2020measure} gives a gentle approach, with an emphasis on students attaining a deep understanding, although I haven't yet looked at that text.
...
\begin{definition}
If $\mathcal{A}$ is a class of sets, the smallest \sf\ containing the sets of $\mathcal{A}$ is written as $\sigma(\mathcal{A}$).  We call this the \textbf{minimal \sf\ over $\mathcal{A}$} or the \textbf{\sf\ generated by $\mathcal{A}$}. 
\label{def:sigma_algebra_generated_by_a_collection}  
\end{definition}



%\begin{definition}
%We call the $\sigma$-algebra of \Eqref{eqn:sigma_algebra_generated_by_a_collection_script_A} the \textbf{$\sigma$-algebra  generated by $\mathcal{A}$}.  This is often denoted $\sigma(\mathcal{A})$.
%\label{def:sigma_algebra_generated_by_a_collection} 
%\end{definition}

Problem~\ref{problem:there_is_a_smallest_sigma_algebra_containing_any_collection} asks us to prove that Def.~\ref{def:sigma_algebra_generated_by_a_collection}   is well-formed.

\begin{problem}\remarktitle{e.g., \cite[Exercise 2.3]{oksendal2013stochastic}}
Given any family $\mathcal{A}$ of subsets of $\Omega$, there is a smallest $\sigma$-algebra containing $\mathcal{A}$, namely
%
\begin{align}
\F_\mathcal{A} \defeq \cap_\alpha \set{\F_\alpha : \F_\alpha \text{ is a $\sigma$-algebra of $\Omega$ }, \mathcal{A} \subset \F_\alpha }
\label{eqn:sigma_algebra_generated_by_a_collection_script_A}
\end{align}
\label{problem:there_is_a_smallest_sigma_algebra_containing_any_collection}
\end{problem}
...
\begin{proof}[Solution to Problem~\ref{problem:there_is_a_smallest_sigma_algebra_containing_any_collection}.]
First we show that $\F_{\mathcal{A}}$  is a $\sigma$-algebra.

\begin{alphabate}
\item $\emptyset \in \F_{\mathcal{A}}? $ \greencheck  We have $\emptyset \in \F_\alpha$ for all $\alpha$.
\item $A \in \F_{\mathcal{A}} \implies A^c \in \F_{\mathcal{A}}$?  \greencheck  We have 
%
\begin{align*}
A \in \F_{\mathcal{A}}  & \explainterm{def. $\F_{\mathcal{A}}$}{\implies} A \in \F_\alpha \qquad \forall \alpha \\
 & \explainterm{$\sigma$-alg}{\implies} A^c \in \F_\alpha \qquad \forall \alpha \\
  & \explainterm{def. $\F_{\mathcal{A}}$}{\implies} A^c \in \F_{\mathcal{A}}
\end{align*}
%
\item $A_n \in \F_{\mathcal{A}} \forall n \implies \cup_n A_n \in \F_{\mathcal{A}}$?  \greencheck  We have
%
\begin{align*}
A_n \in \F_{\mathcal{A}} \quad  \forall n  & \explainterm{def. $\F_{\mathcal{A}}$}{\implies} A_n \in \F_\alpha \qquad \forall \alpha, n \\
 & \explainterm{$\sigma$-alg}{\implies} \cup_n A_n \in \F_\alpha \qquad \forall \alpha \\
  & \explainterm{def. $\F_{\mathcal{A}}$}{\implies} \cup_n A_n \in \F_{\mathcal{A}}
\end{align*}
% 
\end{alphabate}

Now we show that $\F_{\mathcal{A}} $ is the \textit{smallest} algebra containing $\mathcal{A}$.  
\begin{itemize}
\item Suppose $\mathcal{H}$ is another $\sigma$-algebra containing $\mathcal{A}$. We want to show that $ \mathcal{H} \subset 	\F_{\mathcal{A}}$.  But by definition, we have $\mathcal{H} = \F_\alpha \; \text{ for some } \; \alpha$.
\end{itemize}
...
We may define Borel sets more generally, as below.

\begin{definition}\remarktitle{Borel sets in general topological spaces \cite[pp.~8]{oksendal2013stochastic}.}
If $\mathcal{U}$ is a collection of all open sets of any topological space $\Omega$ (e.g. $\Omega=\R^d$), then $\B = \sigma(\mathcal{U})$	 is called the \textit{Borel $\sigma$-algebra} on $\Omega$, and the elements $A \in \sigma(\mathcal{U})$ are called \textit{Borel sets}.
\end{definition}
...
\red{TODO: Add: If $\Omega$ is countable, $\F=2^\Omega$ (i.e. the set of all subsets of $\Omega$) is a sigma-field, and hence any function is measurable. This construction comes up when viewing series as integrals against counting measure.}
...
\begin{example}{\remarktitle{Series as integrals against counting measure.}}
...
\begin{remark}{\remarktitle{Fatou's Lemma for series.}} In the special case where $\Omega = \set{1,2,3, ...}$, $\F=2^\Omega$ (i.e. all subsets of $\Omega$), and $\mu$ is the counting measure, Example~\ref{ex:series_as_integrals} tells us that the integrals are series. That is, we can write $\sum_{k=1}^\infty x_k = \int f \wrt{\mu}$, where $f : \Omega \to \mathcal{X}$ maps $\omega_1, \omega_2, \hdots$ to $x_1, x_2, \hdots$. 
...
\begin{example}
See \url{https://math.stackexchange.com/questions/4719177/a-simple-example-to-practice-justifying-the-interchange-of-derivative-and-expect}.	
\end{example}
...
\begin{exercise}
For some simple exercises applying these definitions on probability spaces, see \citet{pishro2014introduction} (specifically 
\url{https://www.probabilitycourse.com/chapter7/7_2_0_convergence_of_random_variables.php}).	
\end{exercise}


\subsubsection{Properties of modes of convergence}

\begin{proposition}\propositiontitle{On finite measure spaces, convergence in $L^p$ implies convergence in $L^q$.}
Consider a sequence of complex-valued Borel functions $f_1, f_2, \hdots$ on $(\Omega, \F, \mu)$.  Then for any $1 \leq q \leq p < \infty$,
\[ f_n \stackrel{L^p}{\to} f  \quad \implies \quad f_n \stackrel{L^q}{\to} f.\]
\end{proposition}

\begin{proof}
Apply \Holder's inequality to $|fn-f|^q \times 1$ with exponents $\frac{p}{q}$ and  $\frac{p}{p-q}$ to obtain
\[ \int |f_n-f|^q \wrt{\mu} \leq  \bigg(\explaintermbrace{$\to 0$}{\int |f_n-f|^q \wrt{\mu}} \bigg)^{q/p}  \; \big(\explaintermbrace{$< \infty$}{\mu(\Omega)}\big)^{p-q/p}.  \]   	
\end{proof}
...
\subsubsection{The $\sigma$-field induced by a random object} \label{sec:the_sigma_field_induced_by_a_random_object}

Given a random variable $Y$ on $(\Omega, \F)$, one $\sigma$-field $\G \subset \F$ of \textit{particular} interest is the one generated by some \textit{other} random variable $X$ on $(\Omega, \F)$. This will be important later when we study conditional expectations. 
...
\begin{remark}\remarktitle{The $\sigma$-field generated by  a random vector.}
In particular, if $X = (X_1,\hdots,X_n)$ is a random vector, $\sigma(X)$ consists of all sets $\set{X^{-1}(B), B \in \B(\R^n)}$. %{def:sigma_field_generated_by_a_random_object} 
\label{rk:sigma_field_generated_by_a_random_vector}	
\end{remark}

\begin{remark}\remarktitle{Verifying that the $\sigma$-field generated by a random object is a $\sigma$-field.}
...
\begin{example}\remarktitle{The $\sigma$-field induced by a random vector that takes on finitely many values.}
...
\begin{example}	\remarktitle{Any random variable is independent of the trivial $\sigma$-field.}
Let $Y$ be a random variable on $(\Omega, \F)$ and define the sub $\sigma$-field  $\G$ as the trivial $\sigma$-field, that is $\G \defeq \set{\emptyset, \Omega}$.  Then \Eqref{eqn:criterion_for_independence_of_rrandom_variable_and_sigma_field} holds.  For all $B \in \B(\R)$,
%
\begin{align*}
...
\begin{example}\remarktitle{Conditional expectation with respect to a simple $\sigma$-field on the unit square.}
...
\footnotetext{Condition 1 of conditional expectation - namely that $\E[Y \cond \G]$  must be $\G$-measurable - is violated for example because $\set{1} \in \B$, but the inverse image of $\set{1}$ is the top right square, which is not a member of $\G$.}
 
\label{ex:conditional_expectation_with_respect_to_a_simple_sigma_field_on_the_unit_square}
\end{example}

\begin{remark}\remarktitle{Seeing like a $\sigma$-field.}\footnote{I plan to make a slide deck called ``Seeing like a $\sigma$-field", a reference to James Scott's book \textit{Seeing like a state}.} Example~\ref{ex:conditional_expectation_with_respect_to_a_simple_sigma_field_on_the_unit_square} highlights two fundamental points about conditional expectations:
\begin{enumerate}
...
Recall from Sec.~\ref{sec:the_sigma_field_induced_by_a_random_object} the notion of a $\sigma$-field induced by a random variable. Given a random variable $Y$ on $(\Omega, \F)$, one $\sigma$-field $\G \subset \F$ of \textit{particular} interest is the one generated by some \textit{other} random variable $X$ on $(\Omega, \F)$. 
...
& X(x,y)=x \\
& Y(x,y)=y \\
P(E) &= \iint_B f(x,y) \wrt{x} \wrt{y} \quad \forall E \in \B(\R^2) 
\end{align*}
...
\subsection{$\S$ 5.5 Properties of and perspectives on conditional expectation} \label{sec:properties_of_conditional_expectation}
...
\begin{itemize}
\item[(k)] If we take the expectation of a conditional expectation, the result is the same as if we were to take the expectation directly. \red{TODO: Add intuition.}
...
%
But applying MCT to \tinycircled{1}, we also see 
%
...
%
Result \tinycircled{2} and the $\G$-measurability of $h$ reveal that $h=\E[Y \cond \G]$ by the definition of conditional expectation.  Hence, we can rewrite \tinycircled{1} as
...
%
\begin{itemize}
\item[(I)] \boxed{\textnormal{Indicator functions.}} Let $Z = \indicate{\wt{G}}$ be an indicator function for $\wt{G} \in G$.  Then 
\begin{align*}
...
%
\item[(II)] \boxed{\textnormal{Simple functions.}} Let $Z$ be simple, i.e $Z = \sum_{i=1}^r a_i \indicate{G_i}$ for $G_i \in \G$. Then again
\begin{align*}
...
%
\item[(III)] \boxed{\textnormal{Arbitrary measurable functions.}}  By Prop.~\ref{prop:measurable_function_is_limit_of_simple_functions_which_it_dominates},  there exists a sequence of $\G$-measurable simple functions $(S_n)$ such that 
...
\begin{remark}
\cite[pp.229]{durrett2010probability} This result gives a ``geometric interpretation" of $\E[X \cond \G]$ (see Figure~\ref{fig:conditional_expectation_as_projection}).  In particular, $L^2(\Omega, \F, P)$ is a Hilbert space, and $L^2(\Omega, \G, P)$ is a closed subspace.\redfootnote{TODO: Give a proof of this.}  Since $Y=\E[X \cond \G]$ minimizes the (squared) distance function $E(X-Y)^2$, it is the projection of $X$ onto $L^2(\Omega, \G, P)$. That is, it is the point in the subspace closest to $X$. 
\end{remark}
...
\footnotetext{This material is covered in \cite[Thm 6.3.4, 6.3.5]{ash2000probability}.}
...
\begin{remark}
\remarktitle{The line of support theorem does not hold for non-open domains.}
...
We show that \tinycircled{1} cannot hold.
...
\begin{theorem}
Let $g$ be a convex function from $I$ to $\R$, where $I$ is an open interval of reals, bounded or unbounded.  Let $X$ be random variable on $(\Omega, \F, P)$, with $X(\omega) \in I$ for all $\omega$.  Assume $E[X]$ to be finite.\redfootnote{Why do we make this assumption? See Question~\ref{qu:does_jensens_inequality_fail_for_infinite_expectations}.} If $\mathcal{H}$ is a sub $\sigma$-field of $\F$, then 
%
\begin{align}
...
\begin{remark}\remarktitle{Remembering the direction of Jensen's inequality (via a discrete distribution over two outcomes).}
...
Thus, $X=a$ a.e. on $\set{\E[X \cond \mathcal{H}]  \leq a}$. \quad \quad {\scriptsize (by Theorem~\ref{thm:properties_of_functions_derived_from_properties_of_integrals} b)}.

But we assumed $X>a$, so $P\set{\E[X \cond \mathcal{H}]  \leq a} = 0$.  

Thus, $g(\E[X \cond \mathcal{H}] )$ is well-defined.

\item \textit{Now we show that Jensen's inequality (\Eqref{eqn:jensens_inequality_for_conditional_expectations}) holds.} We have 
\begin{align*}
...
\begin{remark}\remarktitle{On representing convex functions with linear functions.}
...
\begin{remark}\remarktitle{Theorems about conditional expectations automatically apply to unconditional expectations.}
...
Where in 1, we 
\begin{itemize}
\item choose $C$ so that $|x_c| \leq \epsilon /2B$ for $c \geq C$ \quad {\scriptsize (possible since $x_c$ converges)}
\item choose $R$ so that $|a_{rc}| \leq \epsilon /2(C \sum_{c=1}^C |x_c|)$ for $r \geq R$ \quad {\scriptsize (possible by condition 1 on $\+A$)}.
\end{itemize}
...
%
According to \cite{ash2000probability}, the second Borel-Cantelli Lemma gives a partial converse. \red{TODO: How is this a converse?}
...
\end{align}
Setting $\set{b_k} = \set{\exp(-P(A_r))}_{r=n}^\infty$, the LHS of  \Eqref{eqn:fact_about_infinite_products} becomes $\sum_{k=1}^\infty -P(A_k) = -\infty$ which holds by hypothesis, so \Eqref{eqn:fact_about_infinite_products} gives that $\prod_{k=n}^\infty \exp(-P(A_k))=0$.  Applying this to \Eqref{eqn:intermediate_equation_for_second_borel_cantelli_lemma}, we find 
...
We substitute this result into \Eqref{eqn:another_intermediate_equation_for_second_borel_equation_lemma} to prove the claim.
\end{proof}
...
\begin{example}\remarktitle{Sample means converge to the population mean for independent random variables with finite means and variance.}
...
Thus, \tinycircled{1} gives us that
...
\subsection{$\S$ 6.3 Martingales  \footnotemark}

\footnotetext{This section seems to only cover \textit{discrete-}time martingales. For continuous-time martingales, see \citet[Ch.~9]{ash2000probability}.}
...
\begin{remark}\remarktitle{
A simple strategy for constructing a filtration.}
...
\begin{example}\remarktitle{A filtration can be constructed from the $\sigma$-fields generated by projecting random sequences of binary values onto their first $n$ coordinates.}
Let $(\Omega = \set{0,1}^\mathbb{N}, \F, P)$ be a probability space of binary sequences.\footnote{In the case where the bits are independent, we can find more detail on $\F$ and $P$, including a justification of the existence of $P$, by examining \cite[Thm.~4.11.1]{ash2000probability}.  \red{TODO: But do we really need to assume independence here? Sec 4.11 on sequences of random variables considers \textit{only} independence and Markov chains. It seems like this restriction should be unnecessary; consider e.g. Gaussian processes.  The construction of infinite product measures in Sec 2.7 seems to be more general. So why does Sec 4.11 exist at all?}}  
...
\begin{example}\remarktitle{Simple random walk.}
...
\begin{remark}\remarktitle{When betting, there is nothing ``super" about a supermartingale.}  If the increments $Y_k$ in Example~\ref{ex:simple_random_walk_as_example_of_martingale_submartingale_or_supermartingle} are taken to be returns in a betting game, then a supermartingale corresponds to an unfavorable game.  In this context, as \citet[pp.232]{durrett2010probability} says, \textit{there is nothing ``super" about a supermartingale.} 

\end{remark}

\begin{example}\remarktitle{An ``increasing information" process.}
...
\begin{remark} \remarktitle{Concrete example of an increasing information process.}
...
%   In the Figure, the conditional expectation gives predictions about the cell values in the $(n+1)$-st subdivided square from the cell values in the $n$-th subdivided square. Criterion (b) requires that our predictions must be constant over the cell values in row $n$. Criterion (a) requires that our prediction for any cell must equal the average of the cell splits given in row $n+1$.   For this reason, our sequence $\set{X_n, \F_n}$ is a martingale; that is, $\E[X_{n+1} \cond \F_n] = X_n$. 
%Recall that $X_n \defeq \E[Y \cond \F_n]$. But the definition of conditional probability demands both average matching and measurability (see Remark~\ref{rk:two_conditions_for_existence_of_conditional_expectation_with_respect_to_a_sigma_field}).
%\red{TODO: Perhaps edit the statements above to try to make intuitive why this example gives a martingale.} 
 \label{rk:concrete_example_of_increasing_information_process} 
\end{remark}

\begin{example}\remarktitle{Polya's Urn Scheme.}
Polya's Urn Scheme is easily shown to be a martingale.  See \cite[pp.241, Sec 5.3.2, first paragraph]{durrett2010probability}.
\end{example}



\begin{proposition}\propositiontitle{A discrete martingale inequality.} \cite[Sec.~2.7.2]{evans2012introduction}


If $\set{X_n}_{n=1}^\infty$ is a submartingale, then 
%
\[ P \bigg(\max_{1 \leq k \leq n} X_k \geq \lambda \bigg) \leq \frac{1}{\lambda} \E[X_n^+]\]
for all $n=1,\hdots$ and $\lambda>0$.
\label{prop:submartingale_chebyshev_inequality}
\end{proposition}

\begin{remark}
Prop.~\ref{prop:submartingale_chebyshev_inequality} can be considered a generalization of Chebyshev's inequality \cite[pp.~35]{evans2012introduction} .  We might refer to it as \enquote{Chebyshev's Inequality for Submartingales}. 
\end{remark}
...
%
\begin{align*}
	A & \defeq \bigg\{\max_{1 \leq k \leq n} X_k \geq \lambda \bigg\} 
\intertext{We want to show}
\lambda P(A) &\leq \E[X_n^+]. 
\end{align*}
%
\begin{align*}
\intertext{Define} 
	A_k & \defeq \bigcap_{j=1}^{k-1} \bigg\{X_j <\lambda \bigg\} \cap \bigg\{X_k \geq \lambda \bigg\} \\
	\intertext{So} 
A &=\bigcupdot_{k=1}^n  A_k 
\intertext{So by countable additivity}
P(A) &= \sum_{k=1}^n P(A_k)  \\
\intertext{Then}
\lambda P(A) &= \lambda \sum_{k=1}^n P(A_k) && \scripttext{Above} \\
&= \lambda \sum_{k=1}^n \int_{A_k} \wrt{P} && \scripttext{Def. integral simple func.} \\
&=  \sum_{k=1}^n \int_{A_k} \lambda \wrt{P} && \scripttext{linearity} \\
&\leq  \sum_{k=1}^n \int X_k \indicate{A_k}\wrt{P} && \scripttext{def $A_k$, notation} \\
&\leq  \sum_{k=1}^n \int \E[X_n \cond X_1, \hdots, X_k] \, \indicate{A_k}\wrt{P} && \scripttext{submartingale (multi-step), monotonicity} \\
&=  \sum_{k=1}^n \E \bigg[\E[X_n \indicate{A_k} \cond X_1, \hdots, X_k]\bigg] && \scripttext{linearity, notation} \\
&=  \sum_{k=1}^n \E[X_n \indicate{A_k}] && \scripttext{Law of Total Expectation} \\
&= \E[X_n \sum_{k=1}^n \indicate{A_k}] && \scripttext{Linearity} \\
& \leq \E[X_n^+ \sum_{k=1}^n \indicate{A_k}] && \scripttext{Monotonicity} \\
& \leq \E[X_n^+]. && \scripttext{Monotonicity}
\end{align*}
...
%
\begin{itemize}
\item To see that $\set{\psi(X_n)}$ is adapted to the filtration $\set{\F_n}$, we begin by noting that $\set{X_n}$ is.  Then note that convex functions $\psi$ are measurable,\redfootnote{TODO: Prove that convex functions are measurable.} and the composition of Borel measurability functions are measurable (Prop.~\ref {prop:a_composition_of_Borel_measurable_functions_is_measurable}).
...
\end{align*}
\footnotetext{$X_n \vee a$ denotes $\max (X_n,a)$.}
...
\begin{remark}\remarktitle{Gambling interpretation of Optional Skipping Theorem.}
...
%or  \cite{durrett2010probability}.  In particular
...
\begin{theorem}\theoremtitle{Upcrossing Theorem.}
%\begin{theorem}\textnormal{\textbf{[Upcrossing Theorem.]}}
...
\begin{remark}\remarktitle{Characterization of integrability in terms of limits.}
\Eqref{eqn:alternate_characterization_of_integrability} can be expressed as 
...
for any measure $\nu$.
% See \cite[Exercise 4.3]{brzezniak2000basic}.	
...
\begin{remark}\remarktitle{Uniform integrability in quantified form.}
...
\phantom{a}
\begin{alphabate}
\item The definition of uniform integrability in \Eqref{eqn:uniform_integrability_in_quantified_form} immediately yields the alternate characterization of integrability (\Eqref{eqn:alternate_characterization_of_integrability}) for each of the members in the sequence.  
\item We can rewrite \Eqref{eqn:decomposing_an_integral_into_a_top_integral_and_bottom_integral} in terms of $f_n$ rather than $f$ to obtain a finite upper bound. Hence, the supremum is also finite.
...
\[|f_n| \leq g \quad \implies  \indicate{|f_n| \geq c} \; \leq  \; \indicate{|g| \geq c} \implies  |f_n| \,  \indicate{|f_n| \geq c} \; \leq  \; |g| \, \indicate{|g| \geq c}  \]
}
...
\subsection{The complex modulus} 
...
The bounds in \Eqref{eqn:bounding_the_complex_modulus_by_real_moduli_appendix}  are very useful.  For instance, from them, we can quickly deduce that the problem of finding limits of sequences of complex numbers can be reduced directly to the real case.
