...
\begin{example}
Let $(\Omega = \set{0,1}^\R, \F, P)$ be a probability space. \red{TODO: What is $\F$ and P here? How do we know that they exist - by Kolmogorov extension?}  %Define the random vector $X_n$ by $\omega \mapsto (\omega_1, \hdots, \omega_n)$, that is, $X_n$ gives the first $n$ coordinates of any $\omega \in \Omega$.  
Define the random variable $Y_n$ by $\omega = (\omega_1, \omega_2, \hdots, \omega_n, \hdots) \mapsto \omega_n$, that is, $Y_n$ gives the $n$-th coordinate of any $\omega \in \Omega$.   Then by the definition of $\sigma$-fields generated by random vectors (see Remark~\ref{rk:sigma_field_generated_by_a_random_vector}), we have that \red{TODO: make the below more explicit, its all countable unions and intersections..}

\begin{itemize}
	\item The $\sigma$-field $\F_1 \defeq \sigma(Y_1)$ partitions $\Omega$ according to the first entry of each outcome; i.e. $\Omega = \Omega_0 \cupdot \Omega_1$, where
%
\begin{align*}
\Omega_0 &\defeq (0, \cdot, \cdot,\cdot, \cdots) \\
\Omega_1 &\defeq (1, \cdot, \cdot,\cdot, \cdots)
\end{align*}
%
\item The $\sigma$-field $\F_2 \defeq \sigma(Y_1, Y_2)$ partitions $\Omega$ according to the first two entries of each outcome; i.e. $\Omega = \Omega_{00} \cupdot \Omega_{01} \cupdot \Omega_{10} \cupdot \Omega_{11}$, where each cell in the partition is defined similarly as above; namely.
%
\begin{align*}
\Omega_{00} &\defeq (0, 0, \cdot,\cdot, \cdots) \\
\Omega_{01} &\defeq (0, 1, \cdot,\cdot, \cdots) \\
\Omega_{10} &\defeq (1, 0, \cdot,\cdot, \cdots) \\
\Omega_{11} &\defeq (1, 1, \cdot,\cdot, \cdots)
\end{align*}
\end{itemize}
Then the sequence $\F_1, \F_2, \hdots$ is a filtration.
 \red{TODO: Finish. This should give an example of a filtration. By defining $Y_1=X_1$, $Y_2=(X_1,X_2)$ (or simply using the $X_n$'s themselves!), we also obtain an example of a sequence of RV's adapted to the filtration.}	
\end{example}
