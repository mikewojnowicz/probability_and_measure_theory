...
\begin{theorem}
Let $g$ be a convex function from $I$ to $\R$, where $I$ is an open interval of reals, bounded or unbounded.  Let $X$ be random variable on $(\Omega, \F, P)$, with $X(\omega) \in I$ for all $\omega$.  Assume $E[X]$ to be finite. \red{why?} If $\mathcal{H}$ is a sub $\sigma$-field of $\F$, then   
...
\red{TODO: Justify the last sentence of the theorem.}	
\end{proof}
