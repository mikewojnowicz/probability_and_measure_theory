...
\[  \explaintermbrace{$\sigma$-field generated by $X$}{\sigma(X)} = \big\{\text{unions of } \;  \set{\Omega_1, \hdots, \Omega_k, \emptyset} \big\}\]
...
\begin{example}\remarktitle{A filtration can be constructed from the $\sigma$-fields generated by projecting random sequences of independent binary values onto their first $n$ coordinates.}
Let $(\Omega = \set{0,1}^\R, \F, P)$ be a probability space of binary sequences.\footnote{More detail on $\F$ and $P$, including a justification of the existence of $P$, can be found by examining \cite[Thm.~4.11.1]{ash2000probability}.}  %Define the random vector $X_n$ by $\omega \mapsto (\omega_1, \hdots, \omega_n)$, that is, $X_n$ gives the first $n$ coordinates of any $\omega \in \Omega$.  
...
Moreover, by defining $X_1=Y_1$, $X_2=(Y_1,Y_2)$ (or simply using the $Y_n$'s themselves!), we also obtain an example of a sequence of RV's adapted to the filtration.	
\end{example}
