...
We now define $\E[Y \cond \G]$ the conditional expectation of a random variable $Y$ with respect to a $\sigma$-field $\G$.   Since an event corresponds to a question that has a yes or no answer, $\E[Y \cond \G]$ can be interpreted as the average value of $Y(\omega)$, given that we know, for each $G \in \G$, whether or not $\omega \in G$.
...
\subsubsection{Conditional expectation given a $\sigma$-field generated by another random variable} 
...
\begin{proof}
See \cite[pp.216]{ash2000probability}. \red{TODO: Give proof. It's just LOTUS.}
\end{proof}
...
So $\E[Y \cond X]$ is constant on vertical strips of $\Omega=\R^2$,\redfootnote{But when is this \textit{not} true?} and the only sets $G \in \sigma(X)$ that we would want to interrogate to obtain knowledge about $Y$ are singleton sets of the form $G=\set{x}, x \in \R$.
\end{example}



%{ex:conditional_expectation_with_random_variables_that_are_discrete_or_absolutely_continuous}
