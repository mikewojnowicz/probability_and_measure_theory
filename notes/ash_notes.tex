...
Although condition (2) always holds for $h=Y$, condition (1) does not generally hold for a   sub-$\sigma$-field $\G \subset \F$. For instance, let $(\Omega, \F) = (\overline{\R}, \B(\overline{\R}))$, and let $Y$ be the identity function.  Define $\G \subsetneq \F$ by $\G = \set{\emptyset, A, A^c, \overline{\R}}$ for $A \subsetneq \overline{\R}$.  Let $B$ be any Borel set that is not in $\G$.  Then $Y^{-1}(B) = B \not\in \G$ and so condition (1) fails. 
...
We say that a random variable $Y$ and a $\sigma$-field $\F$ are independent if 
\begin{align*}
\forall B \in \B(\R), \quad A \in \F: & \\
P\big(\set{Y \in B} \cap A \big) = P\big(\set{Y \in B}) \; P(A)
\end{align*}
...
\item  \red{TODO: Add the Ash example of vertical stripes.}
\end{alphabate}
...
\end{align*}
\red{TODO: Explain why independence holds here.  State failure cases for the other examples.}
\end{alphabate}	
\end{proof}
