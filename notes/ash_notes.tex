...
The probability measure induced by a random variable, as described in  Def.~\ref{def:probability_measure_induced_by_a_random_variable}, is known as a pushforward measure.  Other ways to denote $P_X$ are therefore $X_*(P)$, $X\#P$, or $P \circ X^{-1}$.  	
\end{remark}
...
\stackexplain{Thm.~\ref{thm:relating_lebesgue_and_riemann_integrals}}{=} \int g(x) f(x) \wrt{x} & \stackexplain{Ex.~\ref{ex:integrals_as_sums}}{=} \sum_x g(x) p(x) & \text{derivatives (Prop.~\ref{prop:calculus_with_radon_nikodym_derivatives})}
\end{array}
\]
Note that applying Thm.~\ref{thm:relating_lebesgue_and_riemann_integrals} would require that $gf$ be continuous a.e.; I am not sure how this can hold if we only know that $g$ is measurable.
