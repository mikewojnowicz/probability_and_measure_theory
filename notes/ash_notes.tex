...
\[\E[aY_1 + bY_2 \cond \G] = a \E[Y_1 \cond \G] + b\E[Y_2 \cond \G]. \]
\item[(d)] \textit{(Triangle inequality.)} 
\[ \bigg| \E[Y \cond \G]\bigg| \leq \E\bigg[|Y| \cond \G\bigg] \text{a.e.} \]
\item[(e)] \textit{(The law of total expectation.)}
\[ \E[\E[Y \cond \G]] = \E[Y]\]
\item[(f)] \textit{(Tower property.)}
If $\G_1 \subset \G_2$, then 
	\begin{itemize}
	\item[(i)] $\E[\E[Y \mid \G_2] \mid \G_1] = \E[Y \mid \G_1]$ a.e.
	\item[(ii)] $\E[\E[Y \mid \G_1] \mid \G_2] = \E[Y \mid \G_1]$ a.e.
	\end{itemize}
\item[(g)] \textit{(``Taking out what is known.")}
...
\red{TODO: Explicitly add to the list the conditional versions of dominated convergence, monotone convergence, Fatou's, etc.}

\begin{remark}
Some verbal description of these properties:
\begin{itemize}
\item[(e)] If we take the expectation of a conditional expectation, the result is the same as if we were to take the expectation directly. \red{TODO: Add intuition.}
\item[(f)] This property handles a scenario where we do successive conditioning with respect to two sigma-fields, one of which is coarser than (that is, a subset of) the other.  The observation is that \textit{the smaller $\sigma$-field always wins.}
\item[(g)] This result shows that for conditional expectation with respect to $\G$, random variables $X \in \G$ (i.e. $\G$-measurable random variables) are like constants. They can be brought outside the integral.
\end{itemize}	
\end{remark}
...
\end{array}\]
\item[(e)] By the definition of conditional expectation for $\E[Y \cond \G]$, we have
\[ \int_\Omega \E[Y \cond \G] \wrt{P} = \int_\Omega Y \wrt{P}.  \]
\item[(f)]
\begin{itemize}
\item[(i)] We N.T.S. that the two conditions defining conditional expectation hold. 
\begin{itemize}
...
\end{itemize}
\item[(ii)] We N.T.S. that the two conditions defining conditional expectation hold.  
\begin{itemize}
...
\item[(g)] We N.T.S. that the two conditions defining conditional expectation $\E[YZ \cond \G]$ hold.  
\begin{itemize}
