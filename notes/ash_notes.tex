...
\end{align*}
\label{ex:any_random_variable_is_indepenent_of_the_trivial_sigma_field}
\end{example}
...
\implies & g(X) = a_n X + b_n && \scripttext{$x \in I$} \\
...
\end{align*}
\item \textit{Now we show that the statement holds with unconditional expectations as a special case. } If we set $\mathcal{H} = \set{\emptyset, \Omega}$ to be the trivial $\sigma$-field, then the random variables are independent of $\mathcal{H}$ (see Example~\ref{ex:any_random_variable_is_indepenent_of_the_trivial_sigma_field}), and so the conditionals disappear by the ``no information" rule of conditional expectation (see Example~\ref{ex:examples_with_conditional_expectation_wrt_sigma_field}). 
\end{enumerate}
