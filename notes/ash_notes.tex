...
\[ \G = \Bigg\{ 
 ...
 \[\begin{array}{cccc}
  \begin{tikzpicture}[baseline=+3.0ex, scale=0.4]
   ...
   \node at (1,2) {9};
   \node at (3,2) {20};
 \end{tikzpicture} & ? & \text{\redx} & \text{This violates condition 2 (see Remark~\ref{rk:two_conditions_for_existence_of_conditional_expectation_with_respect_to_a_sigma_field}) of conditional expectation.} \\
 &&& \\
\begin{tikzpicture}[baseline=+3.0ex, scale=0.4]
   \draw         (0,0) rectangle  ++ (4,4);
   \draw         (0,0) rectangle  ++ (2,4);
   ...
   \node at (1,1) {5};
   \node at (3,1) {3};
 \end{tikzpicture} & ? & \text{\redx} & \text{This violates condition 1 (see Remark~\ref{rk:two_conditions_for_existence_of_conditional_expectation_with_respect_to_a_sigma_field}) of conditional expectation.} \footnote{Condition 1 of conditional expectation - namely that $\E[Y \cond \G]$  must be $\G$-measurable - is violated for example because $\set{1} \in \B$, but the inverse image of $\set{1}$ is the top right square, which is not a member of $\G$.} \\
  &&& \\
    \begin{tikzpicture}[baseline=+3.0ex, scale=0.4]
   \draw         (0,0) rectangle  ++ (4,4);
   \draw         (0,0) rectangle  ++ (2,4);
...
\end{align*} 	
%
\red{TODO: Refactor into the filtration section, so that we can more generally speak about filtrations generated by refinements of partitions.}
...
Note that as $n$ increases, $X_n$ gives more information about the values of $Y$ on the square $\Omega$. (In particular, $X_n$ gives us the average value of $Y$ over a grid of sub-rectangles that is a refinement of the corresponding grid over which $X_{n-1}$ gave averages.)  Every time we split a cell in half, the original value $a$ splits into two values $b$ and $c$ such that $a = \frac{b+c}{2}$.  There are of course infinitely many possible choices for $b$ and $c$ given $a$, and we don't know what those values are until we observe the next random variable in the sequence. 

 Going the other way, as $n$ decreases (so we move \textit{up} along Fig.~\ref{fig:example_of_increasing_information_process}), the values of $X_{n-1}$  are \textit{determined} by the values of $X_n$. (In  Fig.~\ref{fig:example_of_increasing_information_process}, the values in a square are determined by the values in the square immediately beneath it.)  So there is no new information given.  This must be true by the definition of conditional probability, which demands both (a) $\int_A \E[X_{n+1} \cond \F_n] \wrt{P} = \int_A X_{n+1} dP$ for all $A \in \F_n$ and  (b) $\E[X_{n+1} \cond \F_n]$ must be $\F_n$-measurable.   In the Figure, the conditional expectation gives predictions about the cell values in the $(n+1)$-st subdivided square from the cell values in the $n$-th subdivided square. Criterion (b) requires that our predictions must be constant over the cell values in row $n$. Criterion (a) requires that our prediction for any cell must equal the average of the cell splits given in row $n+1$.   For this reason, our sequence $\set{X_n, \F_n}$ is a martingale; that is, $\E[X_{n+1} \cond \F_n] = X_n$. \red{TODO: Perhaps edit this paragraph.}  
\end{remark}
