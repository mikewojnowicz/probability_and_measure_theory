...
{\scriptsize (The triangle inequality holds by considering the series as an integral against counting measure.)}
...
where (1) holds by linearity {\scriptsize (e.g. consider these as integrals against counting measure; this holds so long as the RHS does not have the form of  $\pm (\infty - \infty)$, which is avoided since the subtrahend equals $x \in \R$.) }
Thus 
