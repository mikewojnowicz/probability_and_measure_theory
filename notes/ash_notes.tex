...
\begin{remark}
For a gambling interpretation of Theorem~\ref{thm:optional_skipping}, see \citet[pp. 257]{ash2000probability} or \cite{durrett2010probability}.
\end{remark}
...
\end{tikzcd} \]
where we know $X_{1:n}$ is measurable by hypothesis (it is adapted to the filtration) and $\epsilon_n$ is measurable by construction.  Thus, the composition $\wt{\epsilon}_n \defeq \epsilon \circ X_{1:n}$ of measurable functions is measurable.   Now $Y_n$ is defined by algebraic operations on measurable functions, and so is measurable.
...
\begin{definition}
Let $\set{X_n}$ be a sequence of random variables. Let $U_{ab}, \; a,b \in \R, a<b$ be the \textbf{number of upcrossings} of $(a,b)$ by $\set{X_n}$, defined as follows:
...
\end{itemize}
And the number of upcrossings of (a,b) by $\set{X_n}$ is given by $U_{ab}=2$.

\begin{figure}[H]
\centering
\includegraphics[width=.6\textwidth]{images/upcrossings.png}
\caption{\textit{Illustration of number of upcrossings.}  Shown is a realization of  $\set{X_k}_{k=1}^n$ for $n=15$.  The interval of interest $(a,b)$ is given such that $a=0$ and $b$ is designated by a horizontal line. Image credit: \cite[pp.259]{ash2000probability}.}
\label{fig:number_of_upcrossings}
\end{figure}
\end{example}

\begin{theorem}\textnormal{\textbf{[Upcrossing Theorem.]}}
Let $\set{X_k, \F_k}_{k=1}^n$ be a submartingale.  Let $U_{ab}$ be the number of upcrossings of $(a,b)$ by $X_1, \hdots, X_n$.   Then
\[ \E[U_{ab}] \leq \df{\E[(X_k - a)^+]}{b-a} \]	
\label{thm:upcrossing}
\end{theorem}

\begin{proof}
See \cite[pp.258]{ash2000probability}.\footnote{Note that the proof uses Theorem~\ref{thm:optional_skipping}.}
\end{proof}
